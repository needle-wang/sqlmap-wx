...
class Notebook(nb):
  ...
  def build_page1_setting_inject(self, panel, m):
    ...
    m._inject_area_param_ckbtn.Create(_sb, label = '可测试的参数')
    m._inject_area_param_entry.Create(_sb)
    m._inject_area_skip_static_ckbtn.Create(_sb, label = '跳过无动态特性的参数')
    ...
    m._inject_area_skip_ckbtn.Create(_sb, label = '排除参数')
    m._inject_area_skip_entry.Create(_sb)
    m._inject_area_param_exclude_ckbtn.Create(_sb, label = '排除参数(正则)')
    m._inject_area_param_exclude_entry.Create(_sb)
    m._inject_area_dbms_ckbtn.Create(_sb, label = '固定DB类型为')
    ...
    m._inject_area_no_cast_ckbtn.Create(_sb, label = '关掉payload变形机制')
    m._inject_area_no_escape_ckbtn.Create(_sb, label = '关掉string转义')
    m._invalid_label.Create(_sb, label = '对payload中的废值:')
    m._inject_area_invalid_logic_ckbtn.Create(_sb, label = '使用逻辑运算符')
    m._inject_area_invalid_bignum_ckbtn.Create(_sb, label = '使用大数')
    m._inject_area_invalid_str_ckbtn.Create(_sb, label = '使用随机字符串')
  ...
  def build_page1_setting_tech(self, panel, m):
    ...
    m._tech_area_union_chr_ckbtn.Create(_sb, label = '指定枚举列数时所用字符')
    m._tech_area_union_chr_entry.Create(_sb)
  ...
  def build_page1_setting_optimize(self, panel, m):
    ...
    m._optimize_area_null_connect_ckbtn.Create(_sb, label = '只用页面长度报头来比较, 不去获取实际的响应体')
  ...
  def build_page1_setting_general(self, panel, m):
    ...
    m._general_area_verbose_scale.Create(_sb,
                                         ...
                                         maxValue = 6,
                                         style = wx.SL_VALUE_LABEL)
    m._general_area_finger_ckbtn.Create(_sb, label = '执行宽泛的DB版本检测')
    m._general_area_hex_ckbtn.Create(_sb, label = '获取数据时使用hex转换')
  ...
  def build_page1_request_custom(self, panel, m):
    ...
    m._request_area_hpp_ckbtn.Create(_sb, label = '使用HTTP参数污染')
    m._request_area_delay_ckbtn.Create(_sb, label = '请求间隔(秒)')
    m._request_area_delay_entry.Create(_sb)
    m._request_area_timeout_ckbtn.Create(_sb, label = '几秒超时')
  ...
  def build_page1_file_write(self, panel, m):
    ...
    m._file_write_area_udf_ckbtn.Create(_sb, label = '注入(默认sqlmap自带的)用户定义函数')
  ...
  def build_page1_file_os_registry(self, panel, m):
    ...
    m._file_os_registry_reg_key_label.Create(_sb, label = '键')
    m._file_os_registry_reg_key_entry.Create(_sb)
    m._file_os_registry_reg_value_label.Create(_sb, label = '值')
    m._file_os_registry_reg_value_entry.Create(_sb)
    m._file_os_registry_reg_data_label.Create(_sb, label = '数据')
    m._file_os_registry_reg_data_entry.Create(_sb)
    m._file_os_registry_reg_type_label.Create(_sb, label = '类型')
  ...
  def build_page1_other_general(self, panel, m):
    ...
    m._page1_general_binary_fields_ckbtn.Create(_sb, label = '生成有二进制值的字段')
    m._page1_general_binary_fields_entry.Create(_sb)
    m._page1_general_forms_ckbtn.Create(_sb, label = '解析和测试目标url内的表单')
    m._page1_general_parse_errors_ckbtn.Create(_sb, label = '解析并显示DB错误信息')
    m._page1_misc_cleanup_ckbtn.Create(_sb, label = '清理DBMS中sqlmap产生的UDF和表')
    ...
    m._page1_general_session_file_ckbtn.Create(_sb, label = '指定会话文件')
    ...
    m._page1_general_output_dir_ckbtn.Create(_sb, label = '输出的保存目录')
  ...
  def build_page1_other_misc(self, panel, m):
    ...
    m._page1_misc_web_root_ckbtn.Create(_sb, label = '远程web的root目录')
    m._page1_misc_web_root_entry.Create(_sb)
    ...
    m._page1_misc_identify_waf_ckbtn.Create(_sb, label = '鉴别WAF')
    m._page1_misc_skip_waf_ckbtn.Create(_sb, label = '跳过对WAF/IPS保护的启发式侦测')
    m._page1_misc_smart_ckbtn.Create(_sb, label = '只对明显注入点进行详细测试')
    ...
    m._page1_misc_offline_ckbtn.Create(_sb, label = '离线模式(只使用保存的会话数据)')
    m._page1_misc_mobile_ckbtn.Create(_sb, label = '模拟手机请求')
    m._page1_misc_beep_ckbtn.Create(_sb, label = '响铃')
    m._page1_misc_purge_ckbtn.Create(_sb, label = '彻底清除所有记录')
    m._page1_misc_dependencies_ckbtn.Create(_sb, label = '检查丢失的(非核心的)sqlmap依赖')
    m._page1_general_update_ckbtn.Create(_sb, label = '更新sqlmap')
